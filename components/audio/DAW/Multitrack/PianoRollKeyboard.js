...
const PianoRollKeyboard = forwardRef(({
  ...
  const getNoteInfo = (midiNote) => {
    ...
    return { name: noteName, octave, isBlack, fullName: `${noteName}${octave}` };
  };
  ...
  useImperativeHandle(ref, () => ({
    scrollToNote: (note) => {
      // Scroll the keyboard to show a specific note
      const noteHeight = NOTE_HEIGHT * zoom.y;
      const targetY = (MAX_NOTE - note) * noteHeight;
      // Parent component would handle the actual scrolling
    }
  }));
