...
import { trimScore, pitchesToRests, colorMeasures } from '../lib/flat';
...
function ExploratoryCompose({
  ...
  function createJsonFromReference(reference, colorList) {
    let result = pitchesToRests(JSON.parse(reference));
    result = trim ? trimScore(result, trim) : result;
    if (
      colorList &&
      colorList.length > result['score-partwise'].part[0].measure.length
    ) {
      const measures = result['score-partwise'].part[0].measure;
  ...
  useEffect(() => {
    ...
    embed.ready().then(() => {
      ...
      return embed.loadJSON(result).then(() => {
