...
export class ADSREnvelope {
  ...
  scheduleRelease(time) {
    ...
    // Determine if this is a pre-scheduled release (from start with duration)
    // or a manual release (from stop/noteOff)
    const isPreScheduled = time > this.startTime + this.attack + this.decay;

    // Only cancel and re-set current level for manual releases
    // Pre-scheduled releases should let the attack/decay complete naturally
    if (!isPreScheduled) {
      ...
      // Pre-scheduled release: just append to existing envelope
      const sustainLevel = this.sustain * this.peakLevel;
      ...
      if (this.release > 0) {
        this.scheduleCurve(gain, sustainLevel, 0.0001, time, time + this.release, this.releaseCurve);
      } else {
        gain.setValueAtTime(0.0001, time);
      }
    }
