...
import { StringEnsemble } from '../components/audio/DAW/Multitrack/Instruments/WebAudioInstruments';
import BrassEnsemble from './BrassEnsemble';
...
export class EnhancedSynth {
  constructor(audioContext, options = {}) {
    ...
    // Initialize physical modeling instruments
    this.stringEnsemble = new StringEnsemble(audioContext);
    this.stringEnsemble.connect(this.output);
    
    console.log('EnhancedSynth: Creating BrassEnsemble...');
    this.brassEnsemble = new BrassEnsemble(audioContext);
    this.brassEnsemble.connect(this.output);
    console.log('EnhancedSynth: BrassEnsemble created and connected');
    
    console.log('EnhancedSynth: Initialized with wavetables:', this.wavetableSynth.getWavetableNames());
    console.log('EnhancedSynth: Brass ensemble and string physical modeling available');
  }
  ...
  playNote(midiNote, velocity = 1, time = 0, duration = null) {
    ...
    // Use physical modeling for strings
    if (this.options.wavetable === 'strings') {
      // Use the StringEnsemble for realistic string synthesis
      this.stringEnsemble.playNote(midiNote, velocity, time);
      ...
      const voice = {
        ...
        wavetable: 'strings (physical modeling)',
        isActive: true,
      ...
      console.log(`EnhancedSynth: Playing note ${midiNote} with physical modeling strings`);
      return voice;
    }
    
    // Use brass ensemble
    if (this.options.wavetable === 'brass') {
      console.log(`EnhancedSynth: BRASS ENSEMBLE TRIGGERED for note ${midiNote}`);
      // Use the BrassEnsemble for realistic brass section sound
      this.brassEnsemble.playNote(midiNote, velocity, time);
      ...
      this.voices.set(midiNote, voice);
      console.log(`EnhancedSynth: Playing note ${midiNote} with brass ensemble`);
      return voice;
    }
  ...
  stopNote(midiNote, time = 0) {
    ...
    // Handle physical modeling voices
    if (voice.isPhysicalModel) {
      if (voice.modelType === 'strings') {
        this.stringEnsemble.stopNote(midiNote, time);
      } else if (voice.modelType === 'brass') {
        this.brassEnsemble.stopNote(midiNote, time);
      }
      voice.isActive = false;
      this.voices.delete(midiNote);
      console.log(`EnhancedSynth: Stopping physical modeling ${voice.modelType} note ${midiNote}`);
      return;
    }
