...
export class EnhancedSynth {
  ...
  createVoice(midiNote, velocity, time, duration = null) {
    ...
    if (duration !== null && duration > 0) {
      ...
      // ALWAYS schedule oscillator stop for both offline and real-time contexts
      // This ensures notes don't run forever during mixdown
      try {
        osc.stop(actualStopTime);
        console.log(`EnhancedSynth: Scheduled oscillator stop for note ${midiNote} at ${actualStopTime.toFixed(3)}s`);
      } catch (e) {
