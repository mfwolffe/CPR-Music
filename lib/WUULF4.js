...
export default class StringEnsemble {  // Keep class name for compatibility
  constructor(audioContext, options = {}) {
    ...
    this.output.gain.value = 1.2; // Boosted volume to match other instruments
  ...
  setupSpatialProcessing() {
    ...
    combFreqs.forEach(freq => {
      ...
      comb.Q.value = 8; // High Q for hollow resonance
      comb.gain.value = 4;
  ...
  createEtherealVoice(frequency, velocity, time, voiceIndex) {
    ...
    const baseGain = 0.8 / this.options.sectionSize; // Further increased base gain
    ...
    return {
      envelope: envelope,
      oscillators: [...tone.oscillators, lfo],
