...
const effectSliceRegionsWebAudio = async (
  ...
  try {
    ...
    // Update edit history if provided (this will also call setAudioURL)
    if (setEditList) {
      setEditList([newURL]);
    } else {
      // Fallback: if no setEditList provided, just set the URL
      setAudioURL(newURL);
    }

    // Wait for wavesurfer to finish loading (triggered by Waveform.js useEffect)
    if (wavesurfer) {
      console.log('Waiting for wavesurfer to reload via Waveform component...');

      // Stop playback before state change
      wavesurfer.stop();

      // Wait for the ready event (will be fired by Waveform.js's load)
      await new Promise((resolve) => {
        const timeout = setTimeout(() => {
          console.warn('Wavesurfer ready timeout - continuing anyway');
          resolve();
        }, 5000); // 5 second timeout

        wavesurfer.once('ready', () => {
          clearTimeout(timeout);
          console.log('Wavesurfer reload complete via Waveform component');
          resolve();
        });
      });
    }
