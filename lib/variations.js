...
function correctMeasure(measure) {
  ...
  modify &&
    alteredMeasure.note.forEach((note) => {
      note['$adagio-location'].timePos = Math.floor(
...
function mwRhythmicShift(orig, shift) {
  ...
  const updateTimePos = (note) => {
    ...
    const timePos = (note.uPos = note['$adagio-location'].timePos); // updated position
...
function mwMergeVariations(orig, measures) {
  ...
  measures.forEach((measure) => {
    ...
    measure.attributes[0].divisions =
      measure.attributes[0].divisions.toString();
  });
